DROP TABLE ANALYZER_REGISTRY_EXTRA_INFO_TEMP;
DROP TABLE ANALYZER_REGISTRY_TEMP;
...
CREATE TABLE ANALYZER_REGISTRY_EXTRA_INFO(
 ...
 CREATE TABLE ANALYZER_REGISTRY_TEMP
(
	regId bigint not null GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),
	currentDate date,
	currentTime varchar(255),
	...
	vaDmd float,
	analyzerId bigint,
	CONSTRAINT ANALYZER_REGISTRY_TEMP_pk PRIMARY KEY (regId),
	FOREIGN KEY (analyzerId) REFERENCES ANALYZER (analyzerId)
);

CREATE TABLE ANALYZER_REGISTRY_EXTRA_INFO_TEMP(
	regId bigint not null GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),
	kWhNegative float,
	phaseSequence float,
	wDmd float,
	...
	kWhL3Negative float,
	analyzerRegistryId bigint,
	CONSTRAINT ANALYZER_REGISTRY_extra_info_temp_pk PRIMARY KEY (regId),
	FOREIGN KEY (analyzerRegistryId) REFERENCES ANALYZER_REGISTRY (regId)
 );
